...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { event_type, target_type, target_id, device_id } = req.body;
      ...
      await sql`
        INSERT INTO analytics_events (event_type, target_type, target_id, device_id)
        VALUES (${event_type}, ${target_type}, ${target_id}, ${device_id || null})
      `;
    ...
    if (req.method === 'GET') {
      ...
      summary.forEach(row => {
        ...
        if (!grouped[row.target_type][row.target_id]) {
          grouped[row.target_type][row.target_id] = {};
        }
